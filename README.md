...
- **[break-it](./skills/engineering/break-it/SKILL.md)** — Adversarial test-and-assertion discipline: your test's job is to falsify "this code is correct," not confirm it works. State the contract, attack the edges, watch every test fail before trusting it, lock each fixed bug with a regression test at the seam where it hid, and assert invariants in-code so corruption fails loud at the source.
